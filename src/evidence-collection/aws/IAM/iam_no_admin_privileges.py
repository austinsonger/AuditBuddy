...
environments = {
    'private-sector': {
        ...
        'region': 'us-east-1',
        'private_sector_output_file': f"/evidence-artifacts/{current_year}/private-sector/iam_no_admin_privileges.json"
    },
    'federal': {
        ...
        'region': 'us-east-1',
        'federal_output_file': f"/evidence-artifacts/{current_year}/federal/iam_no_admin_privileges.json"
    }
}
...
def check_admin_privileges(user_policies):
    for policy in user_policies:
        if policy['PolicyName'].lower() == 'administratoraccess':
            return True
    return False
...
for env_name, config in environments.items():
    ...
    for user in users_output['Users']:
        ...
        if check_admin_privileges(attached_policies_output['AttachedPolicies']):
            output.append({
                'UserName': user_name,
