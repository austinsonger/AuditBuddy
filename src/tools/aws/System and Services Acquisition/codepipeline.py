...
def main():
    ...
    for env_name, config in environments.items():
        ...
        output_file = f"/evidence-artifacts/{current_year}/{env_name}/aws/configurations/{current_date}.cicd_tool_configuration.json"
        # Define the AWS CLI command with output file path
        aws_command = [
            'aws', 'codepipeline', 'get-pipeline-state', '--region', config.region, '--output', 'json'
        ]
        # Collect evidence
