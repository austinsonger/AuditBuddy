...
def main():
    ...
    for env_name, config in environments.items():
        ...
        output_file = f"/evidence-artifacts/{current_year}/{env_name}/aws/configurations/{current_date}.automatic_test_enforcement.json"
        # Define the AWS CLI command with output file path
        aws_command = [
            'aws', 'codebuild', 'list-projects', '--region', config.region, '--output', 'json'
        ]
        # Collect evidence
