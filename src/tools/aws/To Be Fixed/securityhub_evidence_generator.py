...
environments = {
    'private-sector': {
        ...
        'region': 'us-east-1',
        'private_sector_output_file': f"/evidence-artifacts/{current_year}/private-sector/aws_securityhub_evidence.json"
    },
    'federal': {
        ...
        'region': 'us-east-1',
        'federal_output_file': f"/evidence-artifacts/{current_year}/federal/aws_securityhub_evidence.json"
    }
}
...
for env_name, config in environments.items():
    # Use the setup function to configure the environment
    if setup_aws_environment(config):
        print(f"Environment {env_name} is set up. Processing...")
    ...
    commands = [
        ['aws', 'securityhub', 'list-configuration-policies', '--region', config['region'], '--output', 'json'],
        ['aws', 'securityhub', 'list-security-control-definitions', '--region', config['region'], '--output', 'json'],
...
print("SecurityHub evidence generation complete.")
