...
Description:
    Checks whether the AWS CloudWatch Log Group is encrypted. The rule is NON_COMPLIANT if the CloudWatch has Log Group without encryption enabled.
...
Rule Parameters:
  KmsKeyId (Optional)
    ARN of the KMS key that is used to encrypt the CloudWatch Log Group

Scenarios:
  Scenario: 1
    Given: The rule parameters KmsKeyId has invalid ARN Value.
    Then: return ERROR

  Scenario: 2
    Given: No CloudWatch Log Group is present.
    Then: Return NOT_APPLICABLE

  Scenario: 3
    Given: At least one CloudWatch Log Group is present.
    And: Log Group is not encrypted (DescribeLogGroups call does not return the 'kmsKeyId' name/value pair).
    Then: Return NON_COMPLIANT

  Scenario: 4
    Given: At least one CloudWatch Log Group is present.
    And: Log Group is encrypted (DescribeLogGroups call does not return the 'kmsKeyId' name/value pair).
    And: KmsKeyId parameter is not configured.
    Then: Return COMPLIANT

  Scenario: 5
    Given: At least one CloudWatch Log Group is present.
    And: Log Group is encrypted (DescribeLogGroups call returns the 'kmsKeyId' name/value pair).
    And: KmsKeyId parameter is configured.
    And: KmsKeyId parameter is not matching the KmsKeyId value from the DescribeLogGroups API call.
    Then: Return NON_COMPLIANT
  ...
  Scenario: 6
    ...
    And: Log Group is encrypted (DescribeLogGroups call returns the 'kmsKeyId' name/value pair).
    And: KmsKeyId parameter is configured.
    And: KmsKeyId parameter is matching the KmsKeyId value from the DescribeLogGroups API call.
    Then: Return COMPLIANT
"""
...
def evaluate_compliance(event, configuration_item, valid_rule_parameters):
    ...
    for each_loggroup in all_log_groups:
        # NON_COMPLIANT if kmsKeyId name/value pair does not exist.
        if "kmsKeyId" not in each_loggroup:
            evaluations.append(build_evaluation(each_loggroup['logGroupName'], 'NON_COMPLIANT', event, annotation='This CloudWatch Log Group is not encrypted.'))
            continue
