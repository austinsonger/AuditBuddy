...
Description:
    Checks whether the AWS Lambda function is in a VPC or not. The rule is NON_COMPLIANT if the Lambda function is not in a VPC.
...
Scenarios:
  ...
  Scenario: 2
    Given: Lambda function is present in the VPC i.e. VPC enabled:true
    And: Rule Parameter subnetId is empty
    Then: Return COMPLIANT

  Scenario: 3
    Given: Lambda function is not present in the VPC i.e. VPC enabled:false
    Then: Return NON_COMPLIANT

  Scenario: 4
    Given: Lambda function is present in the VPC i.e. VPC enabled:true
    And: Rule Parameter list doesn't match with the subnetId
    Then: Return NON_COMPLIANT

  Scenario: 5
    Given: Lambda function is present in the VPC i.e. VPC enabled:true
    And: Rule Parameter list matches with the subnetId
    Then: Return COMPLIANT
"""
