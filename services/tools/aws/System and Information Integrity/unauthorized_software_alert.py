...
def main():
    ...
    This script collects evidence of alerts from mechanisms utilized to identify unauthorized software by querying AWS Config and CloudWatch.
    ...
    for env_name, config in environments.items():
        ...
        # Generate the output file path for AWS Config rule evaluations
        output_file_config_evaluations = f"/evidence-artifacts/{current_year}/{env_name}/{current_date}.config_rule_evaluations.json"

        # Define the AWS CLI command for describing AWS Config rule evaluations
        aws_command_config_evaluations = [
            'aws', 'configservice', 'describe-compliance-by-config-rule', '--region', config.region, '--output', 'json'
        ]

        # Collect evidence for AWS Config rule evaluations
        aws_handler.collect_evidence(command_runner, aws_command_config_evaluations, output_file_config_evaluations)

        # Generate the output file path for CloudWatch Alarms
        output_file_cw_alarms = f"/evidence-artifacts/{current_year}/{env_name}/{current_date}.cloudwatch_alarms.json"
        ...
        output_file_cw_alarm_history = f"/evidence-artifacts/{current_year}/{env_name}/{current_date}.cloudwatch_alarm_history.json"
