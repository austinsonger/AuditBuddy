...
def main():
    ...
    for env_name, config in environments.items():
        ...
        output_file = f"/evidence-artifacts/{current_year}/{env_name}/aws/configurations/{current_date}.vpn_split_tunneling_config.json"
        # Define the AWS CLI command with output file path
        aws_command = [
            'aws', 'ec2', 'describe-vpn-connections', '--region', config.region, '--output', 'json'
        ]
        # Collect evidence
