...
def main():
    ...
    for env_name, config in environments.items():
        ...
        output_file = f"/evidence-artifacts/{current_year}/{env_name}/aws/configurations/{current_date}.configuration_management.json"
        # Define the AWS CLI command with output file path
        aws_command = [
            'aws', 'configservice', 'describe-configuration-recorders', '--region', config.region, '--output', 'json'
        ]
        # Collect evidence
