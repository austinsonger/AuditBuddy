...
def main():
    ...
    for env_name, config in environments.items():
        ...
        output_file = f"/evidence-artifacts/{current_year}/{env_name}/lists/{current_date}.codereview.json"
        # Define the AWS CLI command with output file path
        aws_command = [
            'aws', 'codecommit', 'list-repositories', '--region', config.region, '--output', 'json'
        ]
        # Collect evidence
